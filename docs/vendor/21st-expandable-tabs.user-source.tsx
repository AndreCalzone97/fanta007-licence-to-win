// User-supplied source; HTML entities and Markdown escapes normalized.
// https://21st.dev/@victorwelander/components/expandable-tabs — Victor Welander
"use client";
  ...
  return (
    <div ref={outsideClickRef} className={cn("flex flex-wrap items-center gap-2 rounded-2xl border bg-background p-1 shadow-sm", className)}>
      {tabs.map((tab, index) => {
        ...
        const Icon = tab.icon;
        return (
          <motion.button key={tab.title} variants={buttonVariants} initial={false} animate="animate"
            custom={selected === index} onClick={() => handleSelect(index)} transition={transition}
            ...
            <AnimatePresence initial={false}>
              {selected === index && <motion.span variants={spanVariants} initial="initial" animate="animate" exit="exit"
                transition={transition} className="overflow-hidden">{tab.title}</motion.span>}
            </AnimatePresence>
          </motion.button>
        );
      })}
